...
from data.point_cloud_db.shrec import apply_download
...
class TOSCA(PointCloudDataset):
    ...
    def valid_pairs(self,gt_map):
        if(self.hparams.tosca_all_pairs):
            ...
            all = list(itertools.product(list(range(len(self.verts))), list(range(len(self.verts)))))
            all = list(filter(lambda pair: pair[0] != pair[1],all))
            intra = [tuple(int(idx) for idx in k.split('_')) for k in list(gt_map.keys())]
    ...
    @staticmethod
    def load_data(*args):
        tosca_data_path = 'data/datasets/tosca'

        download(tosca_data_path)
        if(not os.path.exists(f"{tosca_data_path}/unified")):
            all_verts,all_faces,all_d_max,all_maps = [],[],[],{}
            sorted_paths = sorted([str(path) for path in list(Path(tosca_data_path).rglob("*.mat"))],key=lambda p:(re.match(r"([a-z]+)([0-9]+)", os.path.basename(p), re.I).groups()[0],int(re.match(r"([a-z]+)([0-9]+)", os.path.basename(p), re.I).groups()[1])))
            ...
            for mat in tqdm(sorted_paths, desc="Unifying tosca"):
                ...
                all_d_max.append(torch.cdist(torch.from_numpy(mesh.vert),torch.from_numpy(mesh.vert)).max())
            gt_names = defaultdict(list)
            for i,mat in enumerate(tqdm(sorted_paths, desc="Unifying gt")):
                name,idx = re.match(r"([a-z]+)([0-9]+)", os.path.basename(mat), re.I).groups()
            ...
            for k in gt_names:
                ...
                for p in all_pairs:
                    all_gt[p] = None
            torch.save((all_verts,all_faces,all_d_max,all_gt),f"{tosca_data_path}/unified")
        else:
            try:
                all_verts,all_faces,all_d_max,all_gt = torch.load(f"{tosca_data_path}/unified", weights_only=False)
            except TypeError:
                all_verts,all_faces,all_d_max,all_gt = torch.load(f"{tosca_data_path}/unified")
