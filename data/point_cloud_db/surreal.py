...
def prepare_surreal_data(split,hparams):
    ...
    if(not os.path.exists(data_path)):
        ...
        # PyTorch>=2.6 default weights_only=True breaks loading full pickled objects
        try:
            datas = torch.load(f"{surreal_data_path}/datas_surreal_{split_for_orig}.pth", weights_only=False)
...
class surreal(PointCloudDataset):
    ...
    def load_data(data_root, split,hparams):
        ...
        try:
            datas = torch.load(data_path, weights_only=False)
        except TypeError:
