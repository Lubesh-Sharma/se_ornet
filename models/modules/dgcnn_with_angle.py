...
class DGCNN_MODULAR(nn.Module):
    ...
    def rotate_point_cloud_by_angle(self, xyz, angles):
        ...
        """
        device = xyz.device
        # Ensure ANGLE tensor is 1-D of angle bins; if a scalar was accidentally passed, rebuild bins
        if self.ANGLE.dim() == 0:
            # Fallback: assume 8 bins
            self.ANGLE = (
                torch.arange(8, dtype=torch.float32, device=device) * 2 * torch.pi / 8
                - torch.pi / 4
            )
        angle_bins = self.ANGLE.to(device)
        rotated_data = torch.zeros_like(xyz, dtype=xyz.dtype, device=device)
        for k in range(xyz.shape[0]):
            toss = angles[k]
            rotation_angle = -angle_bins[toss]
    ...
    def normalize_data(self, batch_data):
        ...
        normal_data = torch.zeros((B, N, C), device=device, dtype=batch_data.dtype)
        for b in range(B):
            pc = batch_data[b]
