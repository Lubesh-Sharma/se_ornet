...
def rotate_by_z_axis(batch_data: torch.Tensor, nbins: int = 8):
    ...
    # Create angle look-up tensor directly on the target device to avoid indexing device mismatch
    ANGLE = (
        torch.arange(nbins, device=device, dtype=batch_data.dtype)
        * 2
        * torch.pi
        / nbins
        - torch.pi / 4
    )
    rotated_gt = torch.randint(nbins, (B,), device=device)
    rotation_angle = ANGLE[rotated_gt]
    rotated_data = torch.zeros_like(batch_data, dtype=batch_data.dtype, device=device)
    for k in range(B):
