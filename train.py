...
def main_train(model_class_pointer, hparams, parser):
    ...
    if hparams.do_train:
        ...
        if hparams.resume_from_checkpoint is not None:
            # PyTorch 2.6 sets torch.load(weights_only=True) by default inside Lightning.
            # Allowlist ModelCheckpoint so unpickling succeeds for trusted checkpoints.
            safe_ctx = getattr(torch.serialization, "safe_globals", None)
            add_safe = getattr(torch.serialization, "add_safe_globals", None)
            try:
                pl_mc = importlib.import_module(
                    "pytorch_lightning.callbacks.model_checkpoint"
                )
                allowed = [pl_mc.ModelCheckpoint]
            except Exception:
                allowed = []

            if safe_ctx and allowed:
                with safe_ctx(allowed):
