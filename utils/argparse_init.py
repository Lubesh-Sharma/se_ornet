...
def init_parse_argparse_default_params(parser, dataset_name=None, arch=None):
    ...
    # max_epochs is already added by pl.Trainer.add_argparse_args in train.py.
    # Avoid duplicate definition conflict: if it exists just override default, else add it.
    if "--max_epochs" in parser._option_string_actions:
        parser.set_defaults(max_epochs={"complition": 200, "shape_corr": 50}[task_name])
    else:
        parser.add_argument(
    ...
    # If Trainer already provided default_root_dir, just override its default
    if "--default_root_dir" in parser._option_string_actions:
        parser.set_defaults(default_root_dir=os.path.join(os.getcwd(), "output", task_name))
    parser.add_argument(
    ...
    # Ensure --gpus not duplicated with pl.Trainer.add_argparse_args
    if "--gpus" in parser._option_string_actions:
    ...
    if "--log_every_n_steps" in parser._option_string_actions:
    ...
    if "--accumulate_grad_batches" in parser._option_string_actions:
